...
namespace cuts {
  ...
  namespace generic {
    inline bool forward_detector_cut(clas12::region_particle* p) {
      int det = abs(p->par()->getStatus());
      // clas12::FD<= det && det < clas12::CD;  // enum missmatch?
      return (2000 <= det && det < 4000);
    }

    inline bool central_detector_cut(clas12::region_particle* p) {
      int det = abs(p->par()->getStatus());
      return (4000 <= det && det < 5000);
    }

    inline bool forward_tagger_cut(clas12::region_particle* p) {
      int det = abs(p->par()->getStatus());
      return (1000 <= det && det < 2000);
    }

    inline bool PID_cut(clas12::region_particle* p, int pid) { return p->par()->getPid() == pid; }

    inline bool charge_cut(clas12::region_particle* p, int charge) { return p->par()->getCharge() == charge; }
  ...
  namespace FD {
    inline bool HTCC_nphe_cut(clas12::region_particle* p) {
    ...
    inline bool EC_sampling_fraction_cut(clas12::region_particle* p, bool inbending = false, bool simulation = false,
    ...
    inline bool EC_hit_position_fiducial_cut_homogeneous(clas12::region_particle* p, cuts::tightness tightness,
                                                         bool inbending) {
    ...
    inline bool EC_outer_vs_EC_inner_cut(clas12::region_particle* p, cuts::tightness tightness) {
    ...
    inline bool DC_fiducial_cut_edge(clas12::region_particle* p, int region, bool inbending) {
      ...
      if (!cuts::generic::PID_cut(p, 11) && !cuts::generic::PID_cut(p, 2212) && !cuts::generic::PID_cut(p, 211) &&
    ...
    inline bool DC_z_vertex_cut(clas12::region_particle* p, bool inbending) {
    ...
    inline bool phot_EC_sampling_fraction_cut(clas12::region_particle* p) {
      bounds limits = {0, 1};
    ...
    inline bool phot_EC_outer_vs_EC_inner_cut(clas12::region_particle* p) {
  ...
  namespace FT {
    ...
    inline bool FT_eid_FTCAL_fiducial_cut(clas12::region_particle* p) {
      double theta = std::acos(p->par()->getPz() / p->par()->getP()) * 180 / std::numbers::pi;

      return theta > 2.5 && theta < 4.5;
    }

    inline bool FT_eid_FTTRK_fiducial_cut(clas12::region_particle* p) {
      // same as FT_eid_FTCAL_fiducial_cut
      return FT_eid_FTCAL_fiducial_cut(p);
    }

    inline bool FT_eid_FTHODO_fiducial_cut(clas12::region_particle* p) {
      // same as FT_eid_FTCAL_fiducial_cut
      return FT_eid_FTCAL_fiducial_cut(p);
    }

    inline bool FT_eid_energy_vs_radius_cut([[maybe_unused]] clas12::region_particle* _) { return true; }

    inline bool FT_photid_FTCAL_fiducial_cut(clas12::region_particle* p) {
    ...
    inline bool FT_photid_beta_cut([[maybe_unused]] clas12::region_particle* _) { return true; }

  }  // namespace FT

  namespace CD {
    inline bool CD_neutr_beta_cut(clas12::region_particle* p, int run) { return cuts::neutr_beta_cut(p, run); }
  }  // namespace CD
  ...
  namespace vertex {
    ...
    inline bool delta_vz_cut(clas12::region_particle* p, double reference_vertex_z) {
    ...
    bool CD_delta_vz_cut(clas12::region_particle* p, double reference_vertex_z) {
      throw not_implemented_error("[CD_delta_vz_cut] This cut is depricated. Use idenical 'delta_vz_cut'.");
  ...
  namespace impl {
    inline bool phot_beta_cut(clas12::region_particle* p, cuts::tightness tightness) {
      if (tightness != 1 and tightness != 2 and tightness != 3)
    ...
    inline bool neutr_beta_cut(clas12::region_particle* p, int run) {
      // very similar to phot_beta_cut
      bounds limits = {0., 0.95};
      return limits.lower < p->par()->getBeta() && p->par()->getBeta() < limits.upper;
    }

    inline bool basic_FTOF_cut(clas12::region_particle* p) {
